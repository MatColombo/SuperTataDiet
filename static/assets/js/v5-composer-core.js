(function (global, factory) {
  ...
  function entryFrom(recipe,version,ingredientById=null){
    if(!recipe||!version)return null;const meta=version.metadata||{},lines=version.ingredientLines||version.ingredient_lines||[],names=[],aliases=[];
    lines.forEach(line=>{const ing=ingredientById?.get?.(line.ingredientId||line.ingredient_id);if(!ing)return;if(ing.name)names.push(ing.name);(ing.aliases||[]).forEach(a=>aliases.push(a));});
    const mealTypes=meta.mealTypes||recipe.mealTypes||[],cuisine=meta.cuisine||recipe.cuisines?.[0]||"",tags=meta.tags||[];
    const searchText=normalize([recipe.title,cuisine,...mealTypes,...tags,...names,...aliases].join(" "));
    return {recipe,version,isCurrent:recipe.currentVersionId===version.id,nutrition:nutritionOfVersion(version),mealTypes,cuisine,tags,ingredientNames:[...new Set(names)],ingredientAliases:[...new Set(aliases)],searchText,prepMinutes:numeric(meta.prepMinutes),mealPrep:meta.mealPrep||{},spiceLevel:meta.spiceLevel||"none",foodGroups:foodPrefs?foodPrefs.groupsForVersion(version,recipe,ingredientById):[]};
  }
  function matchesSearch(entry,term){const q=normalize(term);return !q||String(entry?.searchText||normalize([entry?.recipe?.title,entry?.cuisine,...(entry?.mealTypes||[]),...(entry?.tags||[]),...(entry?.ingredientNames||[]),...(entry?.ingredientAliases||[])].join(" "))).includes(q);}
