(function (global, factory) {
  ...
  function buildPlan(template,startDate,datasetId="tatadiet-base-v2",now=new Date().toISOString()){
  ...
  function applyAction(planInput,daysInput,action,params={},template=null,now=new Date().toISOString()){
    ...
    const afterStats=stats(plan,days); const impact={kind,date:targetDate,before:beforeStats,after:afterStats,affectedDays:Math.max(1,Math.abs(afterStats.dayCount-beforeStats.dayCount)||days.filter((d,i)=>JSON.stringify(d)!==JSON.stringify(originalDays[i])).length),mealsDelta:afterStats.mealCount-beforeStats.mealCount,endDateChange:core.diffDays(beforeStats.end,afterStats.end)};
