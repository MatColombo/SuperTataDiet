...
CURATION=ROOT/'spec/v6/phase-c1-curation.json'
POLICY=ROOT/'spec/v6/phase-c1-policy.json'
...
STAMP='2026-09-08T15:30:00+02:00'
DATASET='tatadiet-base-v2'
PHASE='6.0.0-phase-c1.1'
...
def main():
    ...
    if len(plan.get('days',[]))!=180 or len(cdays)!=180: raise ValueError('C1 requires exactly 180 days')
    selected=[]
    ...
    for day in plan['days']:
        ...
        for meal in day['meals']:
            cm=amap.get(meal['id'])
            if not cm: raise ValueError(f'missing C1 assignment {meal["id"]}')
    ...
    man.setdefault('extensions',{})['v6_phase_c1_curation']={
        'version':PHASE,'source':'spec/v6/phase-c1-curation.json','source_sha256':sha(CURATION),
        'policy_source':'spec/v6/phase-c1-policy.json','policy_sha256':sha(POLICY),
        'recipe_source':'spec/v6/phase-c1-recipes.json','recipe_source_sha256':sha(REC_SPEC),
        'days_reviewed':180,'changed_days_vs_phase_c':cur['summary']['changed_days_vs_phase_c'],
        'changed_meals_vs_phase_c':cur['summary']['changed_meals_vs_phase_c'],
        'note':'Frozen C1 editorial curation: increased real diversity, purchase reuse, and anti-monoculture controls while preserving Phase A/C gates.'
    }
    ...
    dump(MANIFEST,man)
    print(json.dumps({'status':'ok','dataset_version':DATASET,'phase_version':PHASE,'days':180,'meals':len(selected),'changed_meals_vs_phase_c':cur['summary']['changed_meals_vs_phase_c']},ensure_ascii=False,indent=2))
