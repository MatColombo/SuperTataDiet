...
def main() -> None:
    ...
    catalog_extended = bool(recipe_data.get("catalog_extension"))
    if catalog_extended:
        check(ingredient_data.get("generated_at") == plan_data.get("generated_at"), "base-source-timestamps-not-identical", failures)
        check(recipe_data.get("generated_at") == manifest.get("generated_at"), "catalog-extension-timestamps-not-identical", failures)
    else:
        check(len(seed_timestamps) == 1, "base-seed-timestamps-not-identical", failures)
    stable_timestamp = recipe_data.get("generated_at") if catalog_extended else next(iter(seed_timestamps))
    ...
    manifest_recipe_counts = manifest.get("files", {}).get("recipes.base.v1.json", {})
    expected_families = int(manifest_recipe_counts.get("recipe_families", len(families)))
    expected_versions = int(manifest_recipe_counts.get("recipe_versions", len(versions)))
    check(len(families) == expected_families, f"recipe-family-count:{len(families)}", failures)
    check(len(family_by_id) == len(families), "duplicate-recipe-family-id", failures)
    check(len(versions) == expected_versions, f"recipe-version-count:{len(versions)}", failures)
    check(len(version_by_id) == len(versions), "duplicate-recipe-version-id", failures)
    ...
    check(len(structured_versions) + len(manual_versions) == len(versions), f"structured-version-count:{len(structured_versions)}", failures)
