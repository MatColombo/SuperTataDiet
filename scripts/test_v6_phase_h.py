...
def main():
    ...
    assert load('docs/data/build-meta.json')['version']=='6.0.1'
    ...
    assert 'VERSION = "6.0.1"' in build and 'VERSION = "6.0.1"' in validator
    db=text('static/assets/js/v5-db.js'); backup=text('static/assets/js/v5-backup.js'); pwa=text('static/assets/js/pwa.js')
    assert 'const APP_VERSION = "6.0.1";' in db
    assert 'const APP_VERSION = "6.0.1";' in backup
    ...
    assert 'data-version="6.0.1"' in home and 'V6.0.1' in home
    assert 'data-version="6.0.1"' in today
    ...
    report={'status':'ok','phase':'V6-H','release':'6.0.1','checks':{
