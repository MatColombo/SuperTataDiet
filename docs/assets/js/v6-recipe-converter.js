(function(){
  ...
  function recipeMatch(recipes,title,slug){const n=norm(title);let rows=(recipes||[]).filter(r=>!r.archivedAt&&norm(r.title)===n);if(!rows.length)rows=(recipes||[]).filter(r=>norm(r.title).includes(n)||n.includes(norm(r.title)));if(rows.length>1){const s=norm(slug);rows.sort((a,b)=>Number(norm(a.id).includes(s))-Number(norm(b.id).includes(s)));}return rows.at(-1)||null;}
  function renderVersion(){
    const sel=q('[data-converter-version]'),row=q('[data-converter-version-row]');
    if(versions.length>1){row.hidden=false;sel.innerHTML=versions.map((v,i)=>{const n=v.calculatedNutrition||v.manualNutrition||{};return `<option value="${esc(v.id)}"${v.id===version.id?' selected':''}>Versione ${i+1} · ${fmt(n.energyKcal,0)} kcal</option>`;}).join('');}
  ...
  function renderLines(){
    ...
    q('[data-converter-source-summary]').innerHTML=`<div><p class="eyebrow">Ingrediente originale</p><h3>${esc(ing.name)} · ${fmt(quantity(line),1)} ${esc(unit(line))}</h3><p>${fmt(n?.energyKcal,0)} kcal nella ricetta</p></div><div class="converter-mini-nutrition">${nutritionCells(n)}</div>`;
  }
  ...
  function renderComparison(row){
    ...
    box.hidden=false;box.innerHTML=`
      ...
      <div class="converter-explain"><strong>${esc((row.reasons||[]).join(' · ')||'Equivalenza energetica')}</strong>${row.warnings?.length?`<span>${esc(row.warnings.join(' · '))}</span>`:''}<small>Stesse kcal non significano equivalenza nutrizionale o culinaria. Questo strumento non modifica la ricetta.</small></div>`;
  ...
  async function init(){
    try{
      ...
      recipe=recipeMatch(lib.recipes,host.dataset.recipeTitle,host.dataset.recipeSlug);
      ...
      q('[data-converter-version]').onchange=async e=>{version=versions.find(v=>v.id===e.target.value)||version;line=null;selected=null;query='';q('[data-converter-search]').value='';renderLines();await renderAlternatives();};
